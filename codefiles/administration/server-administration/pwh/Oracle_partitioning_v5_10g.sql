...
-- Feature-set/description of our new partitioning implementation V5:
...
--   In case of an update from partitioning V2 or greather to V5, simply remove everything between the "START/END Initial Setup ..."
...
ALTER SESSION SET current_schema=<your_schema>; -- ADAPT THIS PROPERLY. Use the AppMon PWH schema here!
/
...
begin
 highRetentionTimeInDays := 14; -- ADAPT THIS PROPERLY !!!
 P_PARTITIONING('MEASUREMENT_HIGH',7, highRetentionTimeInDays, true, true, true);
 P_PARTITIONING('PERCENTILES_HIGH',7, highRetentionTimeInDays, true, true, true);
end;
...
BEGIN
    ...
    dbms_scheduler.create_job(
        job_name => schema || '.MEASUREMENT_HIGH_PART_JOB_V5',
...
BEGIN
    ...
    SELECT sys_context('USERENV', 'CURRENT_SCHEMA') INTO schema FROM DUAL;
    dbms_scheduler.create_job(
        job_name => schema || '.PERCENTILES_HIGH_PART_JOB_V5',
        job_type => 'PLSQL_BLOCK',
        job_action => schema || '.P_PARTITIONING(''PERCENTILES_HIGH'', 7, ' || highRetentionTimeInDays || ', true, true, true);',
        repeat_interval => 'FREQ=DAILY;BYHOUR=1;BYMINUTE=0;BYSECOND=0',
        start_date => systimestamp at time zone '0:00',
